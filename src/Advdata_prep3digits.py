...
"""def preprocesser(df: pd.DataFrame, preprocessor: TextPreprocessor) -> pd.DataFrame:
    df = preprocessor(df)
    df.loc[:,"2digs"] = df["icd_code"].apply(lambda x: set([y[:2] for y in x]))
    #df.loc[:,"LABELS"] = df["icd_code"].apply(lambda x: ";".join(x))
    df.loc[:,"LABELS"] = df["2digs"].apply(lambda x: ";".join(x))
    df.loc[:,"length"] = df.TEXT.str.count(" ") + 1
    return df
"""
...
##### Load the data
df = load_file_into_df(download_dir / "ints_w_text_to_csv.csv", dtype={"icd_code": str})
numrows = df.shape[0]
...
#m10 = m10.dropna(subset=["LABELS"], how="all")
file = f'Advdata_wordlim{LIMIT}_3digits.csv'
