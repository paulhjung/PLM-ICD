...
code_dict={}
#text2code_dict={}
with open(output_dir_icd10 / 'adv_3_dig_freq_top39.csv') as f:
...
def preprocesser(df: pd.DataFrame, preprocessor: TextPreprocessor) -> pd.DataFrame:
    df = preprocessor(df)
    df.loc[:,"3digs"] = df["icd_code"].apply(lambda x: set([y[:3] for y in x]))
    #df.loc[:,"LABELS"] = df["icd_code"].apply(lambda x: ";".join(x))
    df.loc[:,"LABELS"] = df["3digs"].apply(lambda x: ";".join(x))
    df.loc[:,"length"] = df.TEXT.str.count(" ") + 1
    return df

def top_k_codes(df: pd.DataFrame, column_names: list[str], k: int, l: int) -> set[str]:
    """Get the top k most frequent codes from a dataframe"""
    df = df.copy()
    counter = Counter()
    for col in column_names:
        list(map(counter.update, list(df[col])))
    top_k = counter.most_common(k)
    outfile = "top_k.txt"
    f = open(output_dir_icd10 / outfile, "a")
    for cde in top_k:
        if cde[0] in code_dict.keys() and int(cde[1])*int(code_dict[cde[0]])>l:
            f.write(cde[0]+', '+str(cde[1])+', '+code_dict[cde[0]]+'\n')
        else: print(str(cde)+'\n')
    f.close()
...
top_k_codes(m10, ["3digs"], 350, 180000)
code.interact(local=locals())
m10 = m10.drop(columns=["icd_code","SUBJECT_ID","HADM_ID","3digs"])
...
for i in range(9):
    d = m10[i*k:(i+1)*k]
    fn = f'CHStrain{i}_3digits_wordlim{LIMIT}.csv' 
    d.to_csv(output_dir_icd10 / fn, index=False)
test10 = m10[9*k:]
testfile = f'CHStest_3digits_wordlim{LIMIT}.csv'
