...
from chs3digits_args import parse_args
...
def main():
    output_dir = f"{args.output_prefix}top{args.num_codes}_3digs_max{args.maxtoken_length}_epochs{args.num_overalltrain_epochs}"
    ...
    data_files = {}
    if args.devmode:
        data_files["train"+str(0)] = args.train_file+str(0)+f"_3digits_wordlim{args.wordlimit}.csv"
    elif args.train_file is not None:
        for i in range(9):
            logger.info(f"Loaded dataset {i}")
            data_files["train"+str(i)] = args.train_file+str(i)+f"_3digits_wordlim{args.wordlimit}.csv"
    if args.validation_file is not None:
        data_files["validation"] = args.validation_file+f"_3digits_wordlim{args.wordlimit}.csv"
    ...
    if args.tokens_exist:
        ...
        save_path = DIRECTORY_PLM + f'tokens_3digits_max{args.maxtoken_length}'
    ...
    if args.num_train_epochs > 0:
        ...
        for epoch in tqdm(range(args.num_train_epochs)):
            ...
            for t in [.1, .12, .14, .16, .18, .2, .22, .24, .26]: #these are the cutoffs of the logits
