...
def parse_args():
    ...
    parser.add_argument("--validation_file", type=str, default="../data/mimic4/CHSmimic4icd10test25-0852.csv", help="A csv or a json file containing the validation data.")
    parser.add_argument("--code_file", type=str, default="../data/mimic4/top25codes.txt", help="A txt file containing all codes.")
    parser.add_argument("--max_length", type=int, default=768, help="The maximum total input sequence length after tokenization. Seq longer than this are truncated, seq shorter are padded if `--pad_to_max_lengh` is passed.")
    parser.add_argument("--chunk_size", type=int, default=128, help="The size of chunks that we'll split the inputs into")
    #parser.add_argument("--pad_to_max_length", action="store_true", default=False, help="If passed, pad all samples to `max_length`. Otherwise, dynamic padding is used.")
    ...
    parser.add_argument("--model_name_or_path", type=str, default="../models/CHSroberta-mimic4-1024", help="Path to pretrained model or model identifier from huggingface.co/models.")
    ...
    parser.add_argument("--use_slow_tokenizer", action="store_true", default=False, help="If passed, will use a slow tokenizer (not backed by the 🤗 Tokenizers library).")
    parser.add_argument("--cased", action="store_true", default=False, help="equivalent to do_lower_case=False")
    ...
    #parser.add_argument("--code_50", action='store_true', default=False, help="use only top-50 codes")
