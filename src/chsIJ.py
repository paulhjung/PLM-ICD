...
from chsIJdigits_args import parse_args
...
def main():
    output_dir = f"{args.output_prefix}top{args.num_codes}_IJdigs_max{args.maxtoken_length}_epochs{args.num_overalltrain_epochs}"
    ...
    if args.tokens_exist:
        ...
        save_path = DIRECTORY_PLM + f'tokens_IJdigits_max{args.maxtoken_length}'
    ...
    if args.num_train_epochs > 0:
        ...
        for epoch in tqdm(range(args.num_train_epochs)):
            ...
            for t in [.07, .08, .1, .12, .14, .16, .2, .24, .3, .4, .5, .6]: #these are the cutoffs of the logits
