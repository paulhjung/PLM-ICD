...
from chs_dev_args import parse_args
        ...
        for epoch in tqdm(range(args.num_train_epochs)):
            ...
            for step, batch in enumerate(tqdm(eval_dataloader)):
                with torch.no_grad():
                    outputs = model(**batch)
                preds_raw = outputs[1].sigmoid().cpu()
                preds_raw2 = outputs[3].sigmoid().cpu()
                all_preds_raw.extend(list(preds_raw))
                all_preds_raw2.extend(list(preds_raw2))
                all_labels.extend(list(batch["labels"].cpu().numpy()))
                all_attn.extend(list(outputs[2])) #need to dot attn with hidden
            all_preds_raw = np.stack(all_preds_raw)
            all_preds_raw2 = np.stack(all_preds_raw2)
            all_labels = np.stack(all_labels)
            ...
            for t in [0.33, .34, .35, .36, .37, .38, .39, .4, .41, .42]: #these are the cutoffs of the logits
                all_preds = (all_preds_raw > t).astype(int)
                all_preds2 = (all_preds_raw2 > t).astype(int)
                ...
                metrics2 = all_metrics(yhat=all_preds2, y=all_labels, yhat_raw=all_preds_raw2, k=[5,8,15])
                logger.info(f"metrics2 for threshold {t}: {metrics2}")
