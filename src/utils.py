...
class TextPreprocessor:
    def __init__(
        ...
        self.remove_firstwords = remove_firstwords
    ...
    def __call__(self, df):
        ...
        if self.remove_special_characters:
            ...
            df.loc[:,(TEXT_COLUMN)] = df[TEXT_COLUMN].str.replace("[^a-zA-Z0-9 ]", "", regex=True)
        if self.remove_special_characters_mullenbach:
            df.loc[:,(TEXT_COLUMN)] = df[TEXT_COLUMN].str.replace("[^A-Za-z0-9]+", " ", regex=True)
        if self.remove_firstwords:
